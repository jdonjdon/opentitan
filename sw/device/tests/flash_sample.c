...
const volatile uint32_t flash_data[3];
...
bool test_main(void) {
  ...
  if (pwrmgr_testutils_is_wakeup_reason(&pwrmgr, 0)) {
    ...
    LOG_INFO("POR reset");
    uint32_t addr = (uint32_t)flash_data;
    CHECK(flash_ctrl_testutils_write(&flash_ctrl, addr, 0, ASSA,
                                     kDifFlashCtrlPartitionTypeData, 3) == 0);
    ...
    uint32_t addr = (uint32_t)flash_data;
    for(int i = 0; i < 3; ++i) {
      LOG_INFO("flash_data[%d] = %x",i,abs_mmio_read32(addr));
      addr += 4;
    }
